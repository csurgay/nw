...
for (let i=0; i<3; i++) {
    const nic1 = new NIC(`NIC${2*i+1}`, Math.random() * canvas.width, Math.random() * canvas.height, new MAC());
    nic1.ip = new IPaddress(`192.168.1.${2*i+1}`, 24);
    const nic2 = new NIC(`NIC${2*i+2}`, Math.random() * canvas.width, Math.random() * canvas.height, new MAC());
    nic2.ip = new IPaddress(`192.168.1.${2*i+2}`, 24);
...
for (let i=0; i<3; i++) {
    ...
    nic.ip = new IPaddress(`192.168.1.${10+1}`, 24);
